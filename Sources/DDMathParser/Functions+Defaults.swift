...
private let _sin: (Double) -> Double = sin
private let _sinh: (Double) -> Double = sinh
private let _asin: (Double) -> Double = asin
private let _asinh: (Double) -> Double = asinh
private let _cos: (Double) -> Double = cos
private let _cosh: (Double) -> Double = cosh
private let _acos: (Double) -> Double = acos
private let _acosh: (Double) -> Double = acosh
private let _tan: (Double) -> Double = tan
private let _tanh: (Double) -> Double = tanh
private let _atanh: (Double) -> Double = atanh
private let _atan: (Double) -> Double = atan
private let _atan2: (Double, Double) -> Double = atan2

private let _pow: (Double, Double) -> Double = pow
private let _log: (Double) -> Double = log
private let _sqrt: (Double) -> Double = sqrt
private let _floor: (Double) -> Double = floor
private let _ceil: (Double) -> Double = ceil
...
public extension Function {
    ...
    public static let factorial2 = Function(name: "factorial2", evaluator: { state throws -> Double in
        ...
        guard arg1 == _floor(arg1) else { throw MathParserError(kind: .invalidArguments, range: state.expressionRange) }
        
        if arg1.truncatingRemainder(dividingBy: 2) == 0 {
            let k = arg1 / 2
            return _pow(2, k) * k.factorial()
        } else {
            let k = (arg1 + 1) / 2
            
            let numerator = (2*k).factorial()
            let denominator = _pow(2, k) * k.factorial()
    ...
    public static let nthroot = Function(name: "nthroot", evaluator: { state throws -> Double in
        ...
        if arg1 < 0 && arg2.truncatingRemainder(dividingBy: 2) == 1 {
            ...
            let root = _pow(-arg1, 1/arg2)
            return -root
        } else {
            return _pow(arg1, 1/arg2)
        }
    })
    ...
    public static let stddev = Function(name: "stddev", evaluator: { state throws -> Double in
        ...
        return _sqrt(stddev / Double(state.arguments.count))
    })
    ...
    public static let sin = Function(name: "sin", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        return _sin(Function._dtor(arg1, evaluator: state.evaluator))
    })
    ...
    public static let cos = Function(name: "cos", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        return _cos(Function._dtor(arg1, evaluator: state.evaluator))
    })
    ...
    public static let tan = Function(name: "tan", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        return _tan(Function._dtor(arg1, evaluator: state.evaluator))
    })
    ...
    public static let asin = Function(name: "asin", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        return Function._rtod(_asin(arg1), evaluator: state.evaluator)
    })
    ...
    public static let acos = Function(name: "acos", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        return Function._rtod(_acos(arg1), evaluator: state.evaluator)
    })
    ...
    public static let atan = Function(name: "atan", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        return Function._rtod(_atan(arg1), evaluator: state.evaluator)
    })
    ...
    public static let atan2 = Function(name: "atan2", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        let arg2 = try state.evaluator.evaluate(state.arguments[1], substitutions: state.substitutions)
        return Function._rtod(_atan2(arg1, arg2), evaluator: state.evaluator)
    })
    ...
    public static let csc = Function(name: "csc", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        let sinArg = _sin(Function._dtor(arg1, evaluator: state.evaluator))
    ...
    public static let sec = Function(name: "sec", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        let sinArg = _cos(Function._dtor(arg1, evaluator: state.evaluator))
        guard sinArg != 0 else { throw MathParserError(kind: .divideByZero, range: state.expressionRange) }
        return 1.0 / sinArg
    })
    ...
    public static let cotan = Function(name: "cotan", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        let sinArg = _tan(Function._dtor(arg1, evaluator: state.evaluator))
        guard sinArg != 0 else { throw MathParserError(kind: .divideByZero, range: state.expressionRange) }
        return 1.0 / sinArg
    })
    ...
    public static let acsc = Function(name: "acsc", evaluator: { state throws -> Double in
        ...
        return Function._rtod(_asin(1.0 / arg1), evaluator: state.evaluator)
    })
    ...
    public static let asec = Function(name: "asec", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        guard arg1 != 0 else { throw MathParserError(kind: .divideByZero, range: state.expressionRange) }
        return Function._rtod(_acos(1.0 / arg1), evaluator: state.evaluator)
    })
    ...
    public static let acotan = Function(name: "acotan", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        guard arg1 != 0 else { throw MathParserError(kind: .divideByZero, range: state.expressionRange) }
        return Function._rtod(_atan(1.0 / arg1), evaluator: state.evaluator)
    })
    ...
    public static let versin = Function(names: ["versin", "vers", "ver"], evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        return 1.0 - _cos(Function._dtor(arg1, evaluator: state.evaluator))
    })
    ...
    public static let coversin = Function(names: ["coversin", "cvs"], evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        return 1.0 - _sin(Function._dtor(arg1, evaluator: state.evaluator))
    })
    ...
    public static let exsec = Function(name: "exsec", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        let cosArg1 = _cos(Function._dtor(arg1, evaluator: state.evaluator))
    ...
    public static let excsc = Function(name: "excsc", evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        let sinArg1 = _sin(Function._dtor(arg1, evaluator: state.evaluator))
    ...
    public static let crd = Function(names: ["crd", "chord"], evaluator: { state throws -> Double in
        ...
        let arg1 = try state.evaluator.evaluate(state.arguments[0], substitutions: state.substitutions)
        let sinArg1 = _sin(Function._dtor(arg1, evaluator: state.evaluator) / 2.0)
        return 2 * sinArg1
    })
